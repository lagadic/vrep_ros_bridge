...
#include <proximity_sensor_handler/Proximity_sensorHandler.h>
#include <v_repLib.h>
...
Proximity_sensorHandler::Proximity_sensorHandler() : GenericObjectHandler(),
    //_handleOfProximitySensor(-1),
    _lastPublishedProximitySensor(0.0),
    _acquisitionFrequency(-1.0)
  {
}

Proximity_sensorHandler::~Proximity_sensorHandler(){
}

unsigned int Proximity_sensorHandler::getObjectType() const {
    return CustomDataHeaders::FORCE_SENSOR_DATA_MAIN;
}

void Proximity_sensorHandler::synchronize(){
...
void Proximity_sensorHandler::handleSimulation(){
    ...
    if ( ((currentSimulationTime - _lastPublishedProximitySensor) >= 1.0/_acquisitionFrequency) ){
		...
		if(read_check > 0){
			...
			_lastPublishedProximitySensor = currentSimulationTime;
		}
...
void Proximity_sensorHandler::_initialize(){
    if (_initialized)
        return;
    ...
    if (CAccess::extractSerializationData(developerCustomData, CustomDataHeaders::PROXIMITY_SENSOR_DATA_MAIN,tempMainData)){
...
//    }
    ...
    _lastPublishedProximitySensor = -1.0;

    _initialized=true;
}

bool Proximity_sensorHandler::endOfSimulation(){

    _initialized=false;
    return(false); // We don't want this object automatically destroyed at the end of simulation
}

PLUGINLIB_EXPORT_CLASS(Proximity_sensorHandler, GenericObjectHandler)
