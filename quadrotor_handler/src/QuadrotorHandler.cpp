...
/*void QuadrotorHandler::setID(int newID){
    _id=newID;
}

int QuadrotorHandler::getID(){
    return(_id);
}*/

/*
void QuadrotorHandler::setAssociatedObject(int objID,int objUniqueID){
    _associatedObjectID=objID;
    _associatedObjectUniqueID=objUniqueID;
}

int QuadrotorHandler::getAssociatedObject(){
    return(_associatedObjectID);
}

int QuadrotorHandler::getAssociatedObjectUniqueId(){
    return(_associatedObjectUniqueID);
}

void QuadrotorHandler::synchronizeSceneObject(){
    // We update QuadrotorHandler's associated scene object custom data:
    putQuadrotorTagToSceneObject(_associatedObjectID,_torqueToForceRatio);
}
*/

void QuadrotorHandler::synchronize(){
//    // We update QuadrotorHandler's data from its associated scene object custom data:
...
//    _sub = _nh.subscribe(objectName+"/command", 1000, &QuadrotorHandler::tkMotorCommandsCallback, this);
}


//void QuadrotorHandler::setThrustToForceRatio(float thrustToForceRatio){
...
//}
//
//float QuadrotorHandler::getThrustToForceRatio(){
//    return(_torqueToForceRatio);
//}

//void QuadrotorHandler::putQuadrotorTagToSceneObject(int objectHandle,float torqueToForceRatio){
...
//            std::cout << "Adding " << simGetObjectName(childHandle) << " to exploration list." << std::endl;
    ...
    if (_ctrlMode == CustomDataHeaders::DIRECT){
        ...
        try{
            _sub = _nh.subscribe(objectName+"/command", 1000, &QuadrotorHandler::tkCommandsCallback, this);
        } catch (ros::Exception &e) {
            std::stringstream ss(e.what());
            ss << std::endl;
            ConsoleHandler::printInConsole(ss);
...
void QuadrotorHandler::tkMotorCommandsCallback(const telekyb_msgs::TKMotorCommands::ConstPtr& msg){
    if (msg->force.size() == 4){
...
void QuadrotorHandler::tkCommandsCallback(const telekyb_msgs::TKCommands::ConstPtr& msg){
    _tkCommands = *msg;
    _lastReceivedCmdTime = ros::Time::now();
}
